...
const STATES_AND_LGAS = [
  ...
  { name: 'Imo', code: 'IM', lgas: ['Aboh Mbaise','Ahiazu Mbaise','Ehime Mbano','Ezinihitte','Ideato North','Ideato South','Ihitte/Uboma','Ikeduru','Isiala Mbano','Isu','Mbaitoli','Ngor Okpala','Njaba','Nkwerre','Nwangele','Obowo','Oguta','Ohaji/Egbema','Okigwe','Onuimo','Orlu','Orsu','Oru East','Oru West','Owerri Municipal','Owerri North','Owerri West'] },
  ...
  { name: 'Osun', code: 'OS', lgas: ['Atakumosa East','Atakumosa West','Aiyedaade','Aiyedire','Boluwaduro','Boripe','Ede North','Ede South','Egbedore','Ejigbo','Ife Central','Ife East','Ife North','Ife South','Ifedayo','Ifelodun','Ila','Ilesa East','Ilesa West','Irepodun','Irewole','Isokan','Iwo','Obokun','Odo Otin','Ola Oluwa','Olorunda','Oriade','Orolu','Osogbo'] },
...
async function seed() {
  ...
  const { count } = await supabase.from('states').select('*', { count: 'exact', head: true })
  if (count > 0) {
  ...
  for (const stateData of STATES_AND_LGAS) {
    // Insert state
    const { data: state, error: stateErr } = await supabase
      .from('states')
      .insert({ name: stateData.name, code: stateData.code })
      .select('id')
      .single()

    if (stateErr) {
      console.error(`Failed to insert state ${stateData.name}:`, stateErr.message)
      continue
    }

    // Insert LGAs for this state
    const lgaRows = stateData.lgas.map(name => ({ name, state_id: state.id }))
    const { error: lgaErr } = await supabase.from('lgas').insert(lgaRows)

    if (lgaErr) {
      console.error(`Failed to insert LGAs for ${stateData.name}:`, lgaErr.message)
      continue
    }
  ...
  console.log(`\nDone! Seeded ${STATES_AND_LGAS.length} states and ${totalLgas} LGAs.`)
}
